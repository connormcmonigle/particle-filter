...
struct truncated_representation {
  ...
  [[nodiscard]] constexpr operator T() const noexcept { return integral_component_; }
  [[nodiscard]] constexpr const T& integral_component() const noexcept { return integral_component_; }
  [[nodiscard]] constexpr const float& partial_component() const noexcept { return partial_component_; }

  [[nodiscard]] static inline truncated_representation<T> from_value(const float& value) noexcept {
  ...
  [[nodiscard]] static inline truncated_representation<T>
  ...
  [[nodiscard]] static constexpr truncated_representation<T> from_integral(const T& value) noexcept {
    return truncated_representation<T>{value, float{}};
  }

  [[nodiscard]] static constexpr truncated_representation<T> zero() noexcept { return truncated_representation<T>{T{}, float{}}; }
};
...
struct scaled_truncated_representation_transform {
  ...
  [[nodiscard]] inline truncated_representation<T> operator()(const float& value) noexcept {
...
struct truncated_representation_bounded_plus {
  ...
  [[nodiscard]] inline truncated_representation<T> operator()(
...
class systematic_resampler {
 ...
 private:
  ...
  thrust::omp::vector<particle_type> temp_particles_;
  thrust::omp::vector<index_type> temp_particle_indices_;

  thrust::omp::vector<weight_type> particle_weights_;
  thrust::omp::vector<truncated_representation_type> particle_scatter_indices_;

 public:
  void resample(const thrust::omp::vector<weight_type>& log_weights, thrust::omp::vector<T>& particles) noexcept {
    ...
    thrust::transform(
        ...
        [maximum_log_weight] __device__(const weight_type& log_weight) { return exp(log_weight - maximum_log_weight); });

    const weight_type cumulative_particle_weight =
        thrust::reduce(thrust::omp::par, particle_weights_.cbegin(), particle_weights_.cend());
    ...
    thrust::fill(thrust::omp::par, temp_particle_indices_.begin(), temp_particle_indices_.end(), index_type{});
    ...
    thrust::scatter_if(
        ...
        thrust::make_transform_iterator(
            ...
            [](const truncated_representation_type& index) { return index.integral_component(); }),
