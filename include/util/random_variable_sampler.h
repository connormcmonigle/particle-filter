...
class random_variable_sampler {
 ...
 public:
  ...
  using rng_type = RandomNumberGenerator;
   ...
   void seed(const typename RandomNumberGenerator::result_type& seed) noexcept {
    random_number_generator_.seed(seed);
  }

   [[nodiscard]] T normal_sample(const T& variance) noexcept {
    return sqrt(variance) * standard_normal_(random_number_generator_);
  }

  template <int N>
   [[nodiscard]] Eigen::Matrix<T, N, 1> normal_sample(
   ...
   [[nodiscard]] T unnormalized_normal_log_density(const T& variance, const T& x) const noexcept {
    return -static_cast<T>(0.5) * (x * x) / variance;
  }

  template <int N>
   [[nodiscard]] T unnormalized_normal_log_density(
