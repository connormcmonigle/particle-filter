...
struct device_array {
  ...
  T data_[N];
   ...
   [[nodiscard]] constexpr T& operator[](const std::size_t& idx) noexcept { return data_[idx]; }
   [[nodiscard]] constexpr const T& operator[](const std::size_t& idx) const noexcept { return data_[idx]; }
  ...
  template <typename F>
   [[nodiscard]] inline device_array<typename std::result_of<F(T)>::type, N> transformed(
      F&& f) const noexcept {
  ...
  template <typename F>
   [[nodiscard]] inline device_array<T, N>& selection_sort_by(F&& f) noexcept {
  ...
  template <typename F>
   [[nodiscard]] inline const T* minimum_by(F&& f) const noexcept {
